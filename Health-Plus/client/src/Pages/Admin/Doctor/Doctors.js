...
function Doctors() {
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <div className="action-buttons" style={{ textAlign: 'center' }}>
          <Button variant="info" size="sm" onClick={() => handleViewDoctor(row.original)} style={{ margin: '0 2px' }}>
            <FaEye />
          </Button>
          <Button variant="warning" size="sm" onClick={() => navigate(`/admin/doctors/edit-doctor/${row.original.id}`)} style={{ margin: '0 2px' }}>
            <FaEdit />
          </Button>
  ...
  const currentDoctors = rows.slice(indexOfFirstDoctor, indexOfLastDoctor).map(row => {
    prepareRow(row);
    return row.original;
  });
    ...
    <Container fluid>
      ...
      <Row className="mb-3 mt-3">
        <Col xs={12} md={3} className="mb-2 mb-md-0">
          <Button variant="primary" onClick={() => navigate('/admin/doctors/add-doctor')}>
            <FaPlus /> Add Doctor
          </Button>
        </Col>
            ...
            <tr key={index}>
              {columns.map((column, colIndex) => (
                <td key={colIndex} style={{ width: column.width }}>
                  {column.Cell ? column.Cell({ value: doctor[column.accessor], row: { original: doctor } }) : doctor[column.accessor]}
                </td>
              ))}
            </tr>
          ))}
